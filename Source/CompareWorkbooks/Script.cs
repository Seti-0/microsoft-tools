...
namespace CompareWorkbooks
{
    ...
    public class Script
    {
        ...
        public static void Execute(OfficeApps apps, Input input)
        {
            ...
            input.Template.Copy(After: input.TargetWorkbook.Sheets[apps.Excel.Sheets.Count]);

            if (Flow.Interrupted)
                return;

            Worksheet active = (Worksheet)apps.Excel.ActiveSheet;
            active.Name = ExcelHelper.CreateUniqueWorksheetName(input.TargetWorkbook, "Summary");

            if (Flow.Interrupted)
                return;

            ExcelHelper.TryParseWorksheetRange(out IEnumerable<Worksheet> worksheets, input.TargetWorkbook, input.SourceSheetRange,
                    ...
                    if (match.Success)
                    {
                        ...
                        try
                        {
                            ...
                            foreach (Worksheet sheet in worksheets)
                            {
                                ...
                                ExcelRange sourceCell = sheet.Range[reference];
            ...
            Log.Debug($"({input.OtherWorkbook.FullName} is unchanged)");
